...
var istanbul = require('istanbul'),
    istanbul_coverage = require('istanbul-lib-coverage'),
    hook = istanbul.hook,
...
function mergeClientCoverage(obj) {
    ...
    var map = istanbul_coverage.createCoverageMap({});

    console.log("printar coverage :")
    console.log(coverage)

    console.log("printar OBJ :")
    console.log(obj)

    if(!coverage){
        map.merge(JSON.parse(obj));
    }else {
        map.merge(JSON.parse(coverage));
        map.merge(JSON.parse(obj));
    }
    global.__coverage__ = JSON.stringify(map);
